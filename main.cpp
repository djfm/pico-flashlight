...
static bool under_voltage_protection_enabled = true;

void btn_0_gpio_cb(uint gpio, uint32_t events) {
    ...
    if (events & GPIO_IRQ_EDGE_RISE) {
        if (under_voltage_protection_enabled) {
            printf("disabling under voltage protection\n");
            under_voltage_protection_enabled = false;
        } else {
            printf("enabling under voltage protection\n");
...
int main() {
    ...
    gpio_init(BTN_0);
    gpio_set_irq_enabled_with_callback(BTN_0, GPIO_IRQ_EDGE_RISE, true, &btn_0_gpio_cb);
    gpio_init(LED_0);
    ...
    gpio_put(LED_0, under_voltage_protection_enabled);
    ...
    while (true) {
        ...
        if (under_voltage_protection_enabled && battery_pct < 5) {
            light_on = false;
        }
